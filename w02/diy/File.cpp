...
namespace sdds {
    ...
    // This function is created to read the postal code from the file
    bool read(char* postal)
    {
        return fscanf(fptr, " %[^,],", postal) == 1;
    }

    // This function is created to read the number if population from the file
    bool read(int& population)
    {
        return fscanf(fptr, " %d", &population);
    }
}