/*
   Name - Varun Chhabra
   Email - vchhabra1@myseneca.ca
   StudentID - 170279210
   Date of Completion - 12 November 2022
*/

#define _CRT_SECURE_NO_WARNINGS
#include "Parking.h"

using namespace std;

namespace sdds
{
    // Constructor
    Parking::Parking(const char *filename) : m_parkingMenu("Parking Menu, select an action:", 0),
                                             m_vehicleMenu("Select type of the vehicle:", 1)
    {
        if (filename == nullptr || filename[0] == '\0')
        {
            m_filename = nullptr;
        }
        else
        {
            m_filename = new char[strlen(filename) + 1];
            strcpy(m_filename, filename);
        }
        if (!loadDataFile())
        {
            cout << "Error in data file" << endl;
        }
        m_parkingMenu << "Park Vehicle"
                      << "Return Vehicle"
                      << "List Parked Vehicles"
                      << "Find Vehicle"
                      << "Close Parking (End of day)"
                      << "Exit Program";
        m_vehicleMenu << "Car"
                      << "Motorcycle"
                      << "Cancel";
    }

    //  Destructor
    Parking::~Parking()
    {
        saveDataFile();
        delete[] m_filename;
        m_filename = nullptr;
    }

    // Use to check the empty state
    bool Parking::isEmpty() const
    {
        return m_filename == nullptr;
    }

    // use for printing line
    void Parking::parkingStatus() const
    {
        cout << "****** Valet Parking ******" << endl;
    }

    // Use to print the detail according to the selection
    void Parking::parkVehicle()
    {
        int selection = m_vehicleMenu.run();
        if (selection == 1)
        {
            cout << "---------------------------------" << endl;
            cout << "Parking Car" << endl;
            cout << "---------------------------------" << endl
                 << endl;
        }
        else if (selection == 2)
        {
            cout << "---------------------------------" << endl;
            cout << "Parking Motorcycle" << endl;
            cout << "---------------------------------" << endl
                 << endl;
        }
        else
        {
            cout << "---------------------------------" << endl;
            cout << "Cancelled parking" << endl;
            cout << "---------------------------------" << endl
                 << endl;
        }
    }

    // used for printing the message for returning Vehicle
    void Parking::returnVehicle()
    {
        cout << "---------------------------------" << endl;
        cout << "Returning Vehicle" << endl;
        cout << "---------------------------------" << endl
             << endl;
    }

    // used for printing the message for parked Vehicle
    void Parking::listParkedVehicles() const
    {
        cout << "---------------------------------" << endl;
        cout << "Listing Parked Vehicles" << endl;
        cout << "---------------------------------" << endl
             << endl;
    }

    // used for printing the message for finding Vehicle
    void Parking::findVehicle() const
    {
        cout << "---------------------------------" << endl;
        cout << "Finding a Vehicle" << endl;
        cout << "---------------------------------" << endl
             << endl;
    }

    // this function is used to close the application for the current day and if invalid reponse it will not close
    bool Parking::closeParking()
    {

        char response;
        cout << "This will close the parking; All the vehicles will be removed!" << endl;
        cout << "Are you sure? (Y)es/(N)o: ";
        while (true)
        {
            cin >> response;
            if (response == 'Y' || response == 'y')
            {
                return true;
            }
            else if (response == 'N' || response == 'n')
            {
                return false;
            }
            else
            {
                cout << "Invalid response, only (Y)es or (N)o are acceptable, retry: ";
            }
        }
    }

    // Using this function to exit the application and if invalid reponse it will not exit
    bool Parking::exitParkingApp() const
    {
        char response;
        cout << "This will terminate the application and save the data!" << endl;
        cout << "Are you sure? (Y)es/(N)o: ";
        while (true)
        {
            cin >> response;
            if (response == 'Y' || response == 'y')
            {
                return true;
            }
            else if (response == 'N' || response == 'n')
            {
                return false;
            }
            else
            {
                cout << "Invalid response, only (Y)es or (N)o are acceptable, retry: ";
            }
        }
    }

    // used to load data
    bool Parking::loadDataFile()
    {
        bool ok = false;
        if (m_filename != nullptr)
        {
            cout << "---------------------------------" << endl;
            cout << "loading data from " << m_filename << endl;
            cout << "---------------------------------" << endl
                 << endl;

            ok = true;
        }
        return ok;
    }

    // used to save the file
    void Parking::saveDataFile() const
    {
        if (m_filename != nullptr)
        {
            cout << "---------------------------------" << endl;
            cout << "Saving data into " << m_filename << endl;
            cout << "---------------------------------" << endl
                 << endl;
        }
    }

    // used to redirect the to selected function
    int Parking::run()
    {
        int selection = 0;
        if (!isEmpty())
        {
            bool continueApp = true;
            do
            {
                parkingStatus();
                selection = m_parkingMenu.run();
                switch (selection)
                {
                case 1:
                    parkVehicle();
                    break;
                case 2:
                    returnVehicle();
                    break;
                case 3:
                    listParkedVehicles();
                    break;
                case 4:
                    findVehicle();
                    break;
                case 5:
                    if (closeParking())
                    {
                        continueApp = false;
                        cout << "Ending application!" << endl;
                    }
                    break;
                default:
                    if (exitParkingApp())
                    {
                        continueApp = false;
                        cout << "Exiting application!" << endl;
                    }
                    break;
                }
            } while (continueApp);
            return 0;
        }
        return 1;
    }

}
